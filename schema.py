...
class UserInput(BaseModel):
    ...
    message: str = Field(
        description="User input text to the agent.",
        examples=["What is the weather in San Francisco?"],
    )
    user_id: str = Field(
        description="User id to manage Agent lifecycle for a specific user.",
        examples=["him3008"],
    )
    thread_id: str | None = Field(
        description="Thread ID to persist and continue a multi-turn conversation.",
        default=None,
        examples=["76f514e2-3a22-4ebf-8156-831a6f78f5b9"],
    )
    model: str = Field(
